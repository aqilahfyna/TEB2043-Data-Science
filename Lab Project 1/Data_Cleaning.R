setwd("~/DS Lab/Lab Project 1")
library(tidyverse)
library(lubridate)

raw_lines <- readLines("Uncleaned_Dataset.csv", encoding = "latin1")
raw_lines <- iconv(raw_lines, from = "latin1", to = "UTF-8", sub = "")
data_lines <- raw_lines[-1]
data_lines <- data_lines[data_lines != ""]

# split into pipe rows and comma rows
...
# replace blank and "NA" strings with real NA
df[df == "" | df == "NA" | df == "N/A"] <- NA

# remove pipe rows that didn't parse correctly
...
# fix duplicated name in both columns
